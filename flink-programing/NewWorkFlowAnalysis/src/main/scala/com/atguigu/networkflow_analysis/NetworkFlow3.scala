...
object NetworkFlow3 {
  def main(args: Array[String]): Unit = {
    ...
    // 读取数据
    val inputStream = env.socketTextStream("hadoop202", 7777)
    ...
    val aggStream = dataStream
      ...
      .aggregate( new PageContAgg1(), new PageCountWindowResult1() )
...
class PageContAgg1() extends AggregateFunction[WebServerLogEvent, Long, Long]{
...
class PageCountWindowResult1() extends WindowFunction[Long, PageViewCount, String, TimeWindow]{
...
class TopNPageResult1(n: Int) extends KeyedProcessFunction[Long, PageViewCount, String]{

  // 为了对于同一个key进行更新操作，定义映射状态
  lazy val pageViewCountMapState: MapState[String, Long] = getRuntimeContext
    .getMapState(new MapStateDescriptor[String, Long]("pageViewCount-map", classOf[String], classOf[Long]))
